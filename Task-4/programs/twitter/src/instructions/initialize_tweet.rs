...
pub fn initialize_tweet(
    ...
    if topic.as_bytes().len() > TOPIC_LENGTH {
        return Err(error!(TwitterError::TopicTooLong));
    }
    if content.as_bytes().len() > CONTENT_LENGTH {
        return Err(error!(TwitterError::ContentTooLong));
    }
...
pub struct InitializeTweet<'info> {
    /// Payer and authority for the tweet
    #[account(mut)]
    pub tweet_authority: Signer<'info>,
    /// Tweet PDA: seeds = [topic, TWEET_SEED, tweet_authority]
    #[account(
        init,
